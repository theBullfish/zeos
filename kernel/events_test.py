...
K=os.path.dirname(os.path.abspath(__file__)); B=os.path.join(K,"build"); ESP=os.path.join(B,"espev")
STABLE="/tmp/zeos-stable/BOOTZ.EFI"
QMP="/tmp/zeos-ev-qmp.sock"; SERSOCK="/tmp/zeos-ev-ser.sock"; SERLOG="/tmp/zeos-ev-serial.txt"
...
subprocess.run(["cp","/usr/share/OVMF/OVMF_VARS_4M.fd",os.path.join(B,"OVMF_VARSev.fd")],check=True)
vault=os.path.join(B,"vaultev.img"); subprocess.run(["rm","-f",vault]); subprocess.run(["qemu-img","create","-f","raw",vault,"8M"],check=True)
for p in (QMP,SERSOCK,SERLOG):
    try: os.remove(p)
    except OSError: pass
qemu=subprocess.Popen(["qemu-system-x86_64",*accel_args(),"-machine","q35","-m","512M",
 "-drive","if=pflash,format=raw,readonly=on,file=/usr/share/OVMF/OVMF_CODE_4M.fd",
 "-drive",f"if=pflash,format=raw,file={B}/OVMF_VARSev.fd","-drive",f"format=raw,file=fat:rw:{ESP}",
...
def shot(tag):
    o=f"/tmp/zeos-ev-{tag}.png"; qmp("screendump",filename=o,format="png")
...
time.sleep(1.5)
sw("\n");time.sleep(0.7)
sw("browse test:events\n");time.sleep(4.0)
shot("before")
# click the button (3rd block). content starts ~y=210; h1,p,button stacked.
for cy in (312, 300, 324):
    sw("bclick 165 %d\n"%cy); time.sleep(1.5)
sw("bclick 170 312\n"); time.sleep(1.5)
shot("after")
print("=== SERIAL (events) ===")
print(ser().split("browse test:events")[-1][-700:])
