...
K=os.path.dirname(os.path.abspath(__file__)); B=os.path.join(K,"build"); ESP=os.path.join(B,"espraw")
STABLE="/tmp/zeos-stable/BOOTZ.EFI"
QMP="/tmp/zeos-raw-qmp.sock"; SERSOCK="/tmp/zeos-raw-ser.sock"; SERLOG="/tmp/zeos-raw-serial.txt"
...
subprocess.run(["cp","/usr/share/OVMF/OVMF_VARS_4M.fd",os.path.join(B,"OVMF_VARSraw.fd")],check=True)
vault=os.path.join(B,"vaultraw.img"); subprocess.run(["rm","-f",vault]); subprocess.run(["qemu-img","create","-f","raw",vault,"8M"],check=True)
for p in (QMP,SERSOCK,SERLOG):
    try: os.remove(p)
    except OSError: pass
qemu=subprocess.Popen(["qemu-system-x86_64",*accel_args(),"-machine","q35","-m","512M",
 "-drive","if=pflash,format=raw,readonly=on,file=/usr/share/OVMF/OVMF_CODE_4M.fd",
 "-drive",f"if=pflash,format=raw,file={B}/OVMF_VARSraw.fd","-drive",f"format=raw,file=fat:rw:{ESP}",
...
def shot(tag):
    o=f"/tmp/zeos-raw-{tag}.png"; qmp("screendump",filename=o,format="png")
def key(q):
...
sw(f"bclickp {cx} {cy}\n");time.sleep(1.5)
# REAL keystrokes via QMP input-send-event -> i8042 IRQ -> keyboard.c -> browser field
for qc in ["h","e","l","l","o"]: key(qc)
time.sleep(1.0)
key("ret")
time.sleep(2.0)
shot("done")
