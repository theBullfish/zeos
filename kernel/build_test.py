...
K=os.path.dirname(os.path.abspath(__file__)); B=os.path.join(K,"build"); ESP=os.path.join(B,"espbld")
STABLE="/tmp/zeos-stable/BOOTZ.EFI"
QMP="/tmp/zeos-bld-qmp.sock"; SERSOCK="/tmp/zeos-bld-ser.sock"; SERLOG="/tmp/zeos-bld-serial.txt"
...
subprocess.run(["cp","/usr/share/OVMF/OVMF_VARS_4M.fd",os.path.join(B,"OVMF_VARSbld.fd")],check=True)
vault=os.path.join(B,"vaultbld.img"); subprocess.run(["rm","-f",vault]); subprocess.run(["qemu-img","create","-f","raw",vault,"8M"],check=True)
for p in (QMP,SERSOCK,SERLOG):
    try: os.remove(p)
    except OSError: pass
qemu=subprocess.Popen(["qemu-system-x86_64",*accel_args(),"-machine","q35","-m","512M",
 "-drive","if=pflash,format=raw,readonly=on,file=/usr/share/OVMF/OVMF_CODE_4M.fd",
 "-drive",f"if=pflash,format=raw,file={B}/OVMF_VARSbld.fd","-drive",f"format=raw,file=fat:rw:{ESP}",
...
def shot(tag):
    o=f"/tmp/zeos-bld-{tag}.png"; qmp("screendump",filename=o,format="png")
...
time.sleep(1.5)
sw("\n");time.sleep(0.7)
sw("browse test:build\n");time.sleep(5.0)
shot("build")
print("=== SERIAL (browse test:build) ===")
print(ser().split("browse test:build")[-1][-600:])
