...
- [ ] **‼ H.4** TLS via mbedTLS 1.3 + Mozilla CA bundle — `[BROKEN, observed 2026-08-02]` `https example.com /` resolves DNS (v4+v6) and reaches TCP, but **`mbedtls_ssl_setup` fails with ret=-0x5E80 = MBEDTLS_ERR_SSL_BAD_CONFIG** → "HTTPS fetch failed". Ruled out the obvious causes: `psa_crypto_init()` IS called (net_tls.c:199) and the RNG IS configured (conf_rng, net_tls.c:262). So the mbedTLS 3.x SSL config is incomplete for the enabled TLS version(s) — likely ciphersuite/buffer-length/CA-parse or a TLS-1.3 dependency in mbedtls_config.h. Needs deeper mbedTLS config investigation. `[source net_tls.c:251-355; ca_bundle.h]`. (error message improved to print the ret code.)
