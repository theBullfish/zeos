...
- [x] **B.8** Material vibrancy (ultraThin→ultraThick) — `[VERIFIED/harness]` 5-level material ladder (alpha 140/179/209/230/247 monotonic), fb_material_fill; panel uses MATERIAL_THICK. bible id=446. NOTE: translucency tier, true gaussian blur not implemented. `[source fb.c fb_material_fill]`.
...
- [x] **D.11** Drag desktop icon → chain surface (feed a chain) — `[VERIFIED/harness]` drop over a window feeds its chain the icon payload (wm_surface_at + wm_feed_surface); selftest drag Files->surface 3 fed=Files. bible id=450. `[source desktop.c desktop_drag_end; wm.c wm_feed_surface]`.
...
- [~] **I.7** JavaScript engine + live DOM — `[observed 2026-08-09]` **QuickJS (ES2020) embedded in the kernel** (lib/quickjs + vendored musl-libm lib/zeosm; qjs_port.c freestanding glue). Verified live on real cold boots: (1) **engine** — `js 1+1→2`, `Math.sqrt(2)²→2.0000…4` (real IEEE+libm), arrow fns/closures/`Array.map`, JSON, generators, exceptions; (2) **DOM** — `<script>` executes, `document.getElementById`, `textContent` get/set, `console.log`, page re-renders on mutation (screenshot: `<h1>` shows JS-set text); (3) **events** — `addEventListener`+click dispatch+`event.target`, button click → JS → DOM mutate → repaint (screenshot: counter 0→3 by clicking); (4) **DOM building** — `document.createElement`/`createTextNode`/`appendChild`/`.body`, a script builds a `<ul>` of 3 `<li>` that render (screenshot). `[source lib/quickjs/{quickjs.c,qjs_port.c,qjs_eval.c,qjs_dom.c}; browser.c script exec + DOM API; shell.c cmd_js]`. **REMAINING for "very usable modern web": `fetch()` over the net stack, `querySelector`/`.innerHTML`, broader CSS (fl/grid) + more event types (input/submit), and real-mouse→browser click routing (compositor lane).** Was `[2.0]`; the engine + core DOM/events/build are now real.
