...
- [ ] **A.4** LAPIC-timer preemption (vec 0xEF + setjmp/longjmp), per-chain watchdog — `[UNVERIFIED]` serial shows a preempt fire (`preempted chain 50 after 495950us`) but correctness not asserted. **‼ FIXED critical bug 2026-08-02:** `zeos_longjmp` bypassed IRET and never restored IF, so the first preempt left interrupts DISABLED for the rest of boot → total keyboard/mouse IRQ starvation (this is what killed command-palette + all modal text input). Fix: `sti` on longjmp-resume. bible PALETTE.INPUT id=425. Tick-overrun itself still open.
...
- [ ] **D.13** Dock hover thumbnail, drag-reorder/poof, density size — `[‼ BROKEN]` density recompute triggers a reproducible heisenbug (CR2=0x438/RIP=0 after DESK:initialized); bisected (macro-shadow OK, the dock_apply_density CALL faults), REVERTED to keep tree booting. drag-reorder/poof are pure array ops (safe); hover-thumbnail needs snapshot infra. bible id=457. Needs fault-backtrace session. `[source dock.c]`.
