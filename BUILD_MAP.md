...
- [x] **A.7** Disk encryption / crypto_disk (PIN-gated) — `[VERIFIED/harness]` PIN->AES-XTS-256 armed; region encrypt (cipher differs) + decrypt round-trips exactly. bible id=462. `[source crypto_disk.c crypto_disk_transform]`.
...
- [~] **I.7** JavaScript engine + live DOM — `[observed 2026-08-09]` **QuickJS (ES2020) embedded in the kernel** (lib/quickjs + vendored musl-libm lib/zeosm; qjs_port.c freestanding glue). Verified live on real cold boots: (1) **engine** — `js 1+1→2`, `Math.sqrt(2)²→2.0000…4` (real IEEE+libm), arrow fns/closures/`Array.map`, JSON, generators, exceptions; (2) **DOM query/mutate** — `<script>` executes, `document.getElementById`/`querySelector`/`querySelectorAll`, `textContent`/`innerText`/`innerHTML` get/set, `getAttribute`/`setAttribute`, `className`+`classList.add/remove/toggle/contains`, `.value` (form inputs), `console.log`, page re-renders on mutation (screenshots: `<h1>` JS-set text; `test:dom2` innerHTML rebuild; `test:dom3` class/value); (3) **events** — `addEventListener`+click dispatch (bubbling)+`event.target`, button click → JS → DOM mutate → repaint (screenshot: counter 0→3 by clicking); (4) **DOM build/tree** — `document.createElement`/`createTextNode`/`appendChild`/`.body`, `Element.remove()`, `.children`/`.parentNode`/`.firstElementChild`/`.lastElementChild`/`.next+previousElementSibling` (screenshots: `<ul>` built from a loop; `test:dom3` remove drops a node from the render tree; traversal `first=alpha last=gamma`); (5) **fetch()** — real HTTP over the net stack, returns a Promise, `Response.text()`/`.json()`, microtask drain via `JS_ExecutePendingJob` (`test:fetch`→example.com). `[source lib/quickjs/{quickjs.c,qjs_port.c,qjs_eval.c,qjs_dom.c}; browser.c script exec + DOM API; shell.c cmd_js]`. **REMAINING for "very usable modern web" (two items, both outside the DOM lane): broader CSS layout — flexbox/grid (a large separate CSS-engine effort, I.4/I.5 territory); and live keyboard→form-field / `input`/`change`/`submit`/`keydown` events, which need real input routing into browser widgets — blocked on A.6 (compositor masks IRQs ~40ms; that lane owns it).** Was `[2.0]`; engine + full scriptable DOM (query/mutate/build/traverse/events/fetch) are now real.
