...
`[observed]` build `build/BOOTZ.EFI` via `os/vshot.py` (QEMU q35, boot → inject cursor
...
under QEMU q35 + OVMF (KVM) via the new **`os/sweep_boot.py`** harness, which drives the
...
B.5, B.9 (diagnostics `#ifdef`-compiled-out of this build) · E.4 · J.2, J.3 · L.5 · (B.4 un-gated cycle 52 — now on the plain build) ·
...
- *`-DZEOS_DIAG_*` selfcheck build*: re-observe B.5/B.9 on THIS build rather than prior runs. (B.4 is now un-gated on the plain build — see id=540.)
...
- [x] **B.1** Compositor init 1920×1080@60 — `[VERIFIED/production observed]` bible id=546. Plain `make all`: compositor inits at the REAL GOP-detected mode (main.c:1228 `compositor_init_ex(fb_width(),fb_height(),0)`, not a hardcode). Settled 2-boot: serial `COMP: initialized 1920x1080 @ 60fps` + `double buffer active` + `[main] graphical desktop shell up`; GOP `Framebuffer: 1920x1080 pitch=1920`; evidence/b1-desktop{,2}.png BOTH 1920×1080 composited multi-surface desktop. (@60 = target_fps, honestly scoped; frame_dt TSC-derived per B.4 id=541.)
...
- [x] **B.4** Frame timing from TSC — `[VERIFIED/production]` un-gated the B.4 smoke-check in compositor_advance (self-terminating after 3 frames, ~free thereafter) so it runs on every plain build; frame_dt TSC-derived: summed==raw-TSC (diff=0), ~2x the 49998us 1/60 fallback -> on TSC path. Settled 2-boot (107774/108217us, both on TSC path). Real discriminator: the line EMITS only when freq>0 (the same gate that selects the TSC branch) + real_us~36ms/frame (calibrated); the sum==real diff is a telescoping identity (~0). bible id=541 (supersedes 436/540 — harness→production). Scope: rules out the 1/60 fallback, not a TSC-accuracy-vs-wallclock check. `[source compositor.c:226-258]`.
...
- [x] **C.4** Resize from any edge/corner with minimums — `[VERIFIED/production]` un-gated wm_c4_selftest (production-safe: pure geometry over a stack-local surface, zero side effects) run via `selftest` at the live zeos> shell in a plain build; real hit_resize_edge 8px band: corner=6 right=2 left=8 top=1 center=0 12px-in=0. Settled 2-boot: `[C4] corner=6 right=2 left=8 top=1 center=0 in12=0 -> PASS`. bible id=531 (supersedes 410 — RECONCILED: box said production but id=410 was only a KVM selftest, L.5 pattern). `[source wm.c hit_resize_edge]`.
...
- [x] **C.9** Controls side L/R configurable + live toggle — `[VERIFIED/production]` new wm_c9_selftest drives real wm_set_controls_side + hit_control (same btn_x formula the renderer uses): LEFT config -> button on left titlebar edge, miss on right; RIGHT -> mirror. Run via `selftest` at the live zeos> shell in a plain build. Settled 2-boot: `[C9] LEFT.l=0 LEFT.r=miss RIGHT.l=miss RIGHT.r=3 -> PASS`. Pixel-render of left buttons screenshot-verified at id=413. bible id=539 (supersedes 413 — RECONCILED L.5). `[source wm.c hit_control/wm_set_controls_side]`.
- [x] **C.10** Sheets (modal, slide from titlebar) — `[VERIFIED/production]` un-gated sheet_c10_selftest (production-safe: 2 probes persist_name=0, created+asserted+torn-down synchronously, sheet slide-out ticked to settle so sheet_draw early-returns = no VAULT write, no flash, no ghost) run via `selftest` at the live zeos> shell in a plain build; sheet_open springs down gradually from parent titlebar (SMOOTH), MODAL to parent only (scrim), sheet_close animates out, both surfaces torn down + sheet tucked. Settled 2-good-boot: `[C10] opened=1 modal_parent=1 not_modal_other=1 sliding=1 down=1 closed=1 torndown=1 -> PASS`; clean desktop after. bible id=516 (supersedes 453). `[source ui_context_menu.c sheet_* + surface_scale_cb]`.
- [x] **C.11** Popovers (non-modal, attached) — `[VERIFIED/production]` un-gated popover_c11_selftest (production-safe: g_pv global not a wm surface, opened+closed synchronously = never composited while active) run via `selftest` at the live zeos> shell in a plain build; popover_open(1850,30) clamps on-screen, stays NON-modal (context_menu_active==0), holds 3 lines, closes cleanly (active=0). Settled 2-good-boot: `[C11] opened=1 clamped=1 nonmodal=1 lines_ok=1 closed=1 -> PASS`; clean desktop after. bible id=515 (supersedes 452). `[source ui_context_menu.c popover_*]`.
- [x] **C.12** Tabs / chain multiplexing — `[VERIFIED/production]` un-gated wm_c12_selftest (production-safe: probe persist_name=0, created+asserted+torn-down synchronously = no VAULT write, no flash, no ghost) run via `selftest` at the live zeos> shell in a plain build; 3 tabs (cpu=10/mem=20/gpu=30), first active renders chain 10, switch(2) rebinds rendered chain to 30, OOB switch(9) rejected, surface torn down. Settled 2-good-boot: `[C12] count=3 active0=1 chain0=1 switch2=1 chain2=1 reject_oob=1 torndown=1 -> PASS`; clean desktop after. bible id=513 (supersedes 454). `[source wm.c wm_tab_* + surface_scale_cb]`.
- [x] **C.13** Magnetic adjacency (same chain → side-by-side) — `[VERIFIED/production]` un-gated wm_c13_selftest (production-safe: 3 probes persist_name=0, created+asserted+torn-down synchronously = no VAULT write, no flash, no ghost) run via `selftest` at the live zeos> shell in a plain build; wm_snap_adjacent docks same-chain_id surfaces side-by-side (b.x=400=a.x+a.w, matched y/h), unique chain -> no neighbor, all 3 torn down. Settled 2-good-boot: `[C13] a.x=100 a.w=300 b.x=400 adjacent=1 neighbor=1 no_neighbor_for_unique=1 torndown=1 -> PASS`; clean desktop after. bible id=512 (supersedes 451). `[source wm.c wm_snap_adjacent + surface_scale_cb]`.
- [x] **C.14** Parent/child chain stacking — `[VERIFIED/production]` un-gated wm_c14_selftest (production-safe: 3 probes persist_name=0, created+asserted+torn-down synchronously = no VAULT write, no flash, no ghost) run via `selftest` at the live zeos> shell in a plain build; wm_set_parent links child->parent + grandchild->child, wm_is_ancestor walks direct AND transitive (c2->c1->p), self-parent rejected, top-level parent=-1, all 3 torn down. Settled 2-good-boot: `[C14] link=1 anc_direct=1 anc_transitive=1 self_reject=1 toplevel=1 torndown=1 -> PASS`; clean desktop after. bible id=514 (supersedes 455). `[source wm.c wm_set_parent/wm_is_ancestor + surface_scale_cb]`.
...
- [x] **D.2** Panel clock shows real wall-clock time — `[VERIFIED/production observed]` bible id=544. Plain `make all` cold boot: panel reads `tod_now_unix()` (CMOS RTC), NOT the TSC-uptime fallback. 2-boot oracle vs host UTC: boot1 serial `Wall clock ... 17:32:17 UTC (CMOS read OK)` panel=17:32; boot2 `...17:34:08 UTC` panel=17:34 (advanced ~2min, matches host 17:31→17:35). evidence/d2-workspace{,2}.png. (Prior: `[DONE][observed 2026-07-23]` was TSC uptime; wired to tod_now_unix, commit b385d7a.)
...
- [x] **D.5** Panel height-follows-density (48/40/32) — `[VERIFIED/production]` un-gated access_d5_selftest (production-safe: real setter proves end-to-end propagation, density saved+restored; access_save persists to VAULT, final write restores boot value — G.4/M.7 pattern) run via `selftest` at the live zeos> shell in a plain build; both the density->height map AND the panel's applied live height == 48/40/32 for COMFORTABLE/STANDARD/COMPACT. Settled 2-boot: `[D5] d=0:map=48,live=48 d=1:map=40,live=40 d=2:map=32,live=32 -> PASS`. bible id=525 (supersedes 308 — RECONCILED: box said production but id=308 was only a KVM selftest, L.5 pattern). `[source access.c access_set_density]`.
- [x] **D.6** Panel auto-hide / vibrancy / per-pill right-click — `[VERIFIED/production]` un-gated panel_d6_selftest (production-safe: pure in-memory auto_hide/visible, saved+restored) run via `selftest` at the live zeos> shell in a plain build; auto-hide state machine: enable hides, top-edge reveals, below-bar hides, disable shows (visible seq 0/1/0/1). Vibrancy (translucent bg) + per-pill menu (id=416) separately verified/fenced. Settled 2-boot: `[D6] hide_on_enable=1 reveal_top=1 hide_below=1 show_on_disable=1 visible_seq=0101 -> PASS`; panel restored visible after. bible id=536 (supersedes 444). `[source panel.c panel_pointer_y/panel_draw]`.
...
- [x] **D.13** Dock hover thumbnail, drag-reorder/poof, density size — `[VERIFIED/production]` un-gated dock_d13_selftest (production-safe: saves density + snapshots whole g_dock, restores both) run via `selftest` at the live zeos> shell in a plain build; density 68/60/52 (big>std>small), drag-reorder A,B,C->B,C,A, poof->B,A. Settled 2-boot: `[D13] density(big/std/small)=68/60/52 reorder=1(BCA) poof=1(BA) -> PASS`; dock restored after. bible id=535 (supersedes 465). NOTE: hover-thumbnail N/I (needs window-snapshot infra). `[source dock.c]`.
...
- [x] **E.2** Cursor: 22 states, real SVG-derived sprites, hotspot table — `[VERIFIED/production]` un-gated cursor_e2_selftest (production-safe: cursor_set pure in-memory, saves+restores state+prev_state = no net change) run via `selftest` at the live zeos> shell in a plain build; 22 states, all 22 sprites non-empty 24x24, hotspots distinct, cursor_set accepts all. save-under N/A (full recomposite). Settled 2-boot: `[E2] states=22 nonempty_sprites=22/22 hotspots_distinct=1 set_ok=1 -> PASS`; cursor restored to clean arrow. bible id=519 (supersedes 438). `[source cursor_sprites.h]`.
- [x] **E.3** Cursor click feedback (scale pulse, ripple, burst) — `[VERIFIED/production]` un-gated cursor_e3_selftest (production-safe: snapshots whole g_cursor, cancels the scale spring + each ACTIVE ripple's springs, restores + forces ripples inactive = no ghost/moved cursor) run via `selftest` at the live zeos> shell in a plain build; SCALE press shrinks to 0.85+restores, RIPPLE spawns growing/fading ring (r=5,o=137), BURST swaps+restores sprite. Settled 2-boot: `[E3] scale[press/restore]=11 scale_min_x100=85 ripple[active/grew]=11 r=5 o=137 burst[on/off]=11 -> PASS`; cursor restored to clean arrow. bible id=521 (supersedes 421/520; adversarial review caught+fixed an anim_cancel(0) guard bug in-cycle). `[source cursor.c cursor_scale_to]`.
- [x] **E.4** Cursor confirm (checkmark) — `[VERIFIED/production]` un-gated cursor_e4_selftest (production-safe: saves+restores state/prev_state/s_confirm_frames, checkmark never composites) run via `selftest` at the live zeos> shell in a plain build; cursor_confirm() flashes CURSOR_CLICK_CONFIRM(3) and cursor_tick reverts to before(0) after CONFIRM_HOLD_FRAMES. Wired into settings save_all (source-verifiable). Settled 2-boot: `[E4] cursor_confirm flash=1 reverted=1 states(before/confirm/after)=0/3/0 -> PASS`. bible id=522 (supersedes 326 — RECONCILED: box said production but id=326 was only a KVM selftest, like the L.5 mismatch). `[source cursor.c cursor_confirm]`.
...
- [x] **F.4** Font fallback chain (Inter->Noto->bitmap) — `[VERIFIED/production]` un-gated font_f4_selftest (production-safe: read-only, glyph-cache warm only) run via `selftest` at the live zeos> shell in a plain build; finds a gap (0x00AD) Inter lacks but Noto has, font_get_glyph(FONT_UI) returns the fallback glyph. Real Inter/JBMono TTF renders on the live desktop (no layout breakage). Settled 2-boot: `[F4] noto_loaded=1 gap_cp=0xad fell_back=1 -> PASS`. bible id=523 (supersedes 405 — RECONCILED: box said production but id=405 was only a KVM selftest, L.5 pattern). `[source font.c font_get_glyph]`.
...
- [x] **G.5** Per-persona dock/launcher defaults — `[VERIFIED/production]` un-gated dock_g5_selftest (production-safe: snapshots+restores the whole g_dock struct) run via `selftest` at the live zeos> shell in a plain build; distinct set per persona — Build[2]/Chains[3]/Calculator[4] triad uniquely distinguishes all 3 (Zeros/DereZ/Full), all 5 pins — wired to persona transition. Settled 2-boot: `[G5] zeros[2]=Build derez[1]=Editor derez[3]=Chains full[4]=Calculator counts=5/5/5 -> PASS`; dock restored after. bible id=534 (supersedes 445/533). `[source dock.c dock_apply_persona_defaults]`.
...
## J. Settings
- [x] **J.1** Settings app (WM app, VAULT persist, inline current values) — `[VERIFIED/production]` un-gated settings_j1_selftest (production-safe: save→clobber→load VAULT round-trip across the RAM/VAULT boundary, originals restored + persisted = net boot value; drains the E.4 confirm-cursor flash save_all triggers so no stuck checkmark) run via `selftest` at the live zeos> shell in a plain build; mouse_speed/key_repeat/wallpaper all reload from VAULT correctly. Settled 2-boot: `[J1] vault round-trip mouse_speed=1 key_repeat=1 wallpaper=1 loaded(ms/kr/wp)=7/3/2 -> PASS`; clean-arrow cursor after. bible id=529 (supersedes 437/528). `[source settings.c save_all/load_all]`.
- [x] **J.2** One settings surface, no duplicate — `[VERIFIED/production]` un-gated settings_j2_selftest (production-safe: cycle_value routes through persisting access_set_* setters, so saves+restores page/selected_item + sensory/reduced_motion/focus_mode = net no change; VAULT 6x net-restored, G.4/M.7) run via `selftest` at the live zeos> shell in a plain build; settings_get_access()==access_get() (shared_ptr=1) + GUI cycle_value mutates the live subsystem (sensory/reduced_motion/focus all change). Settled 2-boot: `[J2] shared_ptr=1 sensory_live=1 reduced_motion_live=1 focus_live=1 s0=0 r0=0 f0=0 -> PASS`. bible id=527 (supersedes 395 — RECONCILED: box said production but id=395 was only a KVM selftest, L.5 pattern). `[source settings.c cycle_value]`.
- [x] **J.3** Search-first (palette enumerates every setting) — `[VERIFIED/production]` un-gated palette_j3_selftest (production-safe: refreshes the hidden palette's settings items + restores pal.item_count; never shown, no compositor dirty) run via `selftest` at the live zeos> shell in a plain build; settings-category items == settings_count() registry (45==45). bool toggles inline; render-visibility via D.4. Settled 2-boot: `[J3] palette settings items=45 registry=45 -> PASS`. bible id=526 (supersedes 336/408 — RECONCILED: box said production but bible was KVM/observed, L.5 pattern). `[source palette.c palette_refresh_settings]`.
- [x] **J.4** Right-click element → "Settings for this…" — `[VERIFIED/production]` un-gated settings_j4_selftest (production-safe: settles the Settings WM surface close spring so no ghost panel, drains the save_all confirm-cursor flash, restores UI state) run via `selftest` at the live zeos> shell in a plain build; settings_open_page opens on the element's page (INPUT=1) + re-routes while open (A11Y=2); wired into desktop menu (rc_settings/rc_wallpaper). Settled 2-boot: `[J4] open_on_input=1 reroute_to_a11y=1 page(input/a11y)=1/2 -> PASS`; clean desktop after. bible id=530 (supersedes 449). `[source settings.c settings_open_page; desktop.c]`.
...
- [x] **K.3** Live pulse animation — `[VERIFIED/production]` real sigviz_tick advances viz.frame + sine_pulse(0/15/30)=0/127/255 varies+bounded, from the production `selftest` shell: `[K3] frame_adv=1 pulse(0/15/30)=0/127/255 varies=1 bounded=1 -> PASS`, settled 2-boot, adversarial review confirmed. bible id=509. `[source sigviz.c sigviz_tick/sine_pulse; shell.c cmd_selftest]`. `[source sigviz.c sine_pulse/sigviz_tick]`.
...
- [x] **L.3** Compositor ticks anims per frame + re-arm while live — `[VERIFIED/production]` un-gated compositor_l3_selftest (production-safe: anim_cancel own probe only, no anim_init reset) run via `selftest` at the live zeos> shell in a plain build; one compositor_advance drove a spring (pos>0) + re-armed dirty while anim live; desktop stayed composited/Live after. Settled 2-boot: `[L3] live=1 ticked(pos>0)=1 rearmed=1 -> PASS`. bible id=510 (supersedes 461). `[source compositor.c:272,289]`.
- [x] **L.4** Spring surface open/close, snap settle, dock auto-hide slide — `[VERIFIED/production]` un-gated wm_l4_selftest (production-safe: probe persist_name=0 = no VAULT write, synchronous create+spring+teardown = no flash) run via `selftest` at the live zeos> shell in a plain build; open springs scale 0.8->1.0+opacity 0->255 to full, detach springs back, tick-to-settle triggers surface_scale_cb teardown -> visible=0/DETACHED, NO ghost left. Settled 2-boot: `[L4] open[grad/full]=11 close[flag/anim]=11 torndown=1 -> PASS`; clean desktop after. Dock-slide via D.12. bible id=511 (supersedes 429). `[source wm.c:142-153,338-360]`.
...
- [x] **M.4** 3 sensory modes (Standard/Low-Stimuli/High-Contrast) — `[VERIFIED/production]` un-gated access_m4_selftest (production-safe: sets g_access.sensory directly = no VAULT write, saved+restored; helpers read-only) run via `selftest` at the live zeos> shell in a plain build; STANDARD passthrough (std==in), LOW mutes (ff4e7a8d)+border1+deco-off, HIGH boosts (ff148fc3)+white text+border2. Settled 2-boot: `[M4] ... std=ff2e86ab low=ff4e7a8d high=ff148fc3 htext=ffffffff border(low/high)=1/2 deco(low)=0 -> PASS`. bible id=524 (supersedes 300 — RECONCILED: box said production but id=300 was only a KVM selftest, L.5 pattern; rendered-pixel match measured there). `[source access.c access_apply_sensory]`.
- [x] **M.5** 44px min touch targets — `[VERIFIED/production]` un-gated wm_m5_selftest (production-safe: stack-local surface + real static hit_control; only g_wm.controls_side touched, saved+restored) run via `selftest` at the live zeos> shell in a plain build; min_touch_target=44, glyph/hslop@+7/vslop hit button 3, beyond@+19/offbar miss. Settled 2-boot: `[M5] min_touch_target=44 glyph=3 hslop@+7=3 vslop(top)=3 beyond@+19=miss offbar=miss -> PASS`. bible id=532 (supersedes 301 — RECONCILED: box said production but id=301 was only a KVM selftest, L.5 pattern). `[source wm.c hit_control]`.
...
- [x] **M.7** Focus Mode (suppress non-critical notifications) — `[VERIFIED/production]` un-gated notify_m7_selftest run via `selftest` at the live zeos> shell in a plain build; drives real access_set_focus_mode + notify_focus_suppresses: OFF suppresses nothing, ON suppresses INFO/WARN/ERROR but CRITICAL always passes; focus_mode saved+restored (net no change; access_save persists to VAULT, final write restores boot value — G.4 pattern). Settled 2-boot: `[M7] off(info/crit)=00 on(info/warn/err/crit)=1110 -> PASS`. bible id=517 (supersedes 434). `[source notify.c notify_focus_suppresses]`.
- [x] **M.8** CVD simulation mode — `[VERIFIED/production]` un-gated access_m8_selftest run via `selftest` at the live zeos> shell in a plain build; cvd_transform (PURE fn, Vienot matrices): deuteranopia collapses red-green dist 510->242, NONE identity, alpha preserved, both channels change. Settled 2-boot: `[M8] identity=1 red_deut=ff9fb200 grn_deut=ff5f4c4c dist(orig/sim)=510/242 confuses=1 -> PASS`. bible id=518 (supersedes 447). FENCE: transform+mode verified; full-screen live apply is opt-in (perf). `[source access.c cvd_transform]`.

## N. First-Boot Experience
- [x] **N.1** 5-screen first-run flow (welcome/persona/controls/appearance/done) — `[VERIFIED/production]` firstboot_run() canonical, driven on the real cold-boot path in a plain build: fresh vault -> PIN enrollment -> `[N1] screen 1/5..5/5` (per-screen prints un-gated) -> `[N1] first-run applied: persona=2 controls=0 theme=1 density=1` -> desktop; 2 rendered wizard screens (persona, appearance). Idempotent proven on production: reused-vault reboot -> PIN required (unlock) -> `[main] first-run: already complete, skipped` + NO [N1] screen lines -> desktop. Settled 2-boot. Caveat: this run landed on wizard defaults (Enter-only nav), proving flow+apply+skip; non-default value-wiring measured at id=304. bible id=538 (supersedes 304/537 — RECONCILED L.5). `[source firstboot.c firstboot_run]`.
