                     ...
                     ql_read_fn read_fn, void *ctx)
{
    ...
    /* Fallback sniff: if the host read_fn returned nothing but the
     * path looks like a FAT32 path and FAT32 is mounted, do our own
     * header read. Keeps Quick Look self-sufficient when the calling
     * surface (desktop, palette, future file picker) hasn't wired
     * a content reader yet. */
    if (g_ql.buf_used == 0 && path && path[0] == '/' && fat32_mounted()) {
        struct fat32_file f;
        if (fat32_open(path, &f) >= 0) {
            if (g_ql.size == 0) g_ql.size = f.size;
            int n = fat32_read(&f, g_ql.buf, QL_SNIFF_SIZE);
            if (n > 0) g_ql.buf_used = n;
        }
    }

    if (g_ql.buf_used > 0) {
        g_ql.kind = ql_sniff(g_ql.buf, g_ql.buf_used);
        if (g_ql.kind == QL_KIND_PNG) {
            ql_parse_png_dims(g_ql.buf, g_ql.buf_used,
                              &g_ql.png_w, &g_ql.png_h);
