...
# drives dns/ping (v4 regression check) + ping6 (SLAAC/v6).  in slirp.
...
sw("\n");time.sleep(0.8)
for c in ['js 1+1','js Math.sqrt(2)*Math.sqrt(2)','js [1,2,3].map(x=>x*x).join(",")','js JSON.stringify({a:1,b:[2,3]})','js (function*(){yield 7})().next().value','js "abc".toUpperCase()+(2**10)']:
    sw(c+"\n"); time.sleep(2.5)
print("=== OUTPUT ===")
print(ser()[-1400:])
