#!/usr/bin/env python3
# I.7 DOM proof: boot (stable EFI), `browse test:dom3` — a page whose <script>
...
K=os.path.dirname(os.path.abspath(__file__)); B=os.path.join(K,"build"); ESP=os.path.join(B,"espnt")
STABLE="/tmp/zeos-stable/BOOTZ.EFI"
QMP="/tmp/zeos-nt-qmp.sock"; SERSOCK="/tmp/zeos-nt-ser.sock"; SERLOG="/tmp/zeos-nt-serial.txt"
...
subprocess.run(["cp","/usr/share/OVMF/OVMF_VARS_4M.fd",os.path.join(B,"OVMF_VARSnt.fd")],check=True)
vault=os.path.join(B,"vaultnt.img"); subprocess.run(["rm","-f",vault]); subprocess.run(["qemu-img","create","-f","raw",vault,"8M"],check=True)
for p in (QMP,SERSOCK,SERLOG):
    try: os.remove(p)
    except OSError: pass
qemu=subprocess.Popen(["qemu-system-x86_64",*accel_args(),"-machine","q35","-m","512M",
 "-drive","if=pflash,format=raw,readonly=on,file=/usr/share/OVMF/OVMF_CODE_4M.fd",
 "-drive",f"if=pflash,format=raw,file={B}/OVMF_VARSnt.fd","-drive",f"format=raw,file=fat:rw:{ESP}",
...
def shot(tag):
    o=f"/tmp/zeos-nt-{tag}.png"; qmp("screendump",filename=o,format="png")
...
sw('hwnote\n');time.sleep(1.4)
sw('hwnote share\n');time.sleep(1.4)
sw('hwnote add 17cb 0308 xhci Q6A-USB-verified-on-metal\n');time.sleep(1.4)
