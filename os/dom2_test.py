#!/usr/bin/env python3
# I.7 DOM proof: boot (stable EFI), `browse test:dom2` — a page whose <script>
...
K=os.path.dirname(os.path.abspath(__file__)); B=os.path.join(K,"build"); ESP=os.path.join(B,"espd2")
STABLE="/tmp/zeos-stable/BOOTZ.EFI"
QMP="/tmp/zeos-d2-qmp.sock"; SERSOCK="/tmp/zeos-d2-ser.sock"; SERLOG="/tmp/zeos-d2-serial.txt"
...
subprocess.run(["cp","/usr/share/OVMF/OVMF_VARS_4M.fd",os.path.join(B,"OVMF_VARSd2.fd")],check=True)
vault=os.path.join(B,"vaultd2.img"); subprocess.run(["rm","-f",vault]); subprocess.run(["qemu-img","create","-f","raw",vault,"8M"],check=True)
for p in (QMP,SERSOCK,SERLOG):
    try: os.remove(p)
    except OSError: pass
qemu=subprocess.Popen(["qemu-system-x86_64",*accel_args(),"-machine","q35","-m","512M",
 "-drive","if=pflash,format=raw,readonly=on,file=/usr/share/OVMF/OVMF_CODE_4M.fd",
 "-drive",f"if=pflash,format=raw,file={B}/OVMF_VARSd2.fd","-drive",f"format=raw,file=fat:rw:{ESP}",
...
def shot(tag):
    o=f"/tmp/zeos-d2-{tag}.png"; qmp("screendump",filename=o,format="png")
...
sw("browse test:dom2\n");time.sleep(5.0)
shot("jsdom")
print("=== SERIAL (browse test:dom2) ===")
print(ser().split("browse test:dom2")[-1][-600:])
