...
K=os.path.dirname(os.path.abspath(__file__)); B=os.path.join(K,"build"); ESP=os.path.join(B,"espsub")
STABLE="/tmp/zeos-stable/BOOTZ.EFI"
QMP="/tmp/zeos-sub-qmp.sock"; SERSOCK="/tmp/zeos-sub-ser.sock"; SERLOG="/tmp/zeos-sub-serial.txt"
...
subprocess.run(["cp","/usr/share/OVMF/OVMF_VARS_4M.fd",os.path.join(B,"OVMF_VARSsub.fd")],check=True)
vault=os.path.join(B,"vaultsub.img"); subprocess.run(["rm","-f",vault]); subprocess.run(["qemu-img","create","-f","raw",vault,"8M"],check=True)
for p in (QMP,SERSOCK,SERLOG):
    try: os.remove(p)
    except OSError: pass
qemu=subprocess.Popen(["qemu-system-x86_64",*accel_args(),"-machine","q35","-m","512M",
 "-drive","if=pflash,format=raw,readonly=on,file=/usr/share/OVMF/OVMF_CODE_4M.fd",
 "-drive",f"if=pflash,format=raw,file={B}/OVMF_VARSsub.fd","-drive",f"format=raw,file=fat:rw:{ESP}",
...
def shot(tag):
    o=f"/tmp/zeos-sub-{tag}.png"; qmp("screendump",filename=o,format="png")
def key(q):
...
time.sleep(1.5)
sw("\n");time.sleep(0.7)
sw("browse test:submit\n");time.sleep(4.0)
# query the LIVE (post-layout) button box, click its center
sw("bbox go\n");time.sleep(1.5)
m=re.search(r"bbox go x=(\d+) y=(\d+) w=(\d+) h=(\d+)", ser())
if not m:
    print("NO BBOX"); print(ser()[-800:]); _stop[0]=True; qmp("quit"); sys.exit(1)
bx,by,bw,bh=[int(g) for g in m.groups()]
cx,cy=bx+bw//2,by+bh//2
print("button box:",bx,by,bw,bh,"-> click",cx,cy,flush=True)
sw(f"bclickp {cx} {cy}\n");time.sleep(3.0)
shot("done")
tail=ser().split("browse test:submit")[-1]
print("=== submit result ===")
