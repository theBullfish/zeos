#!/usr/bin/env python3
# I.7 DOM proof: boot (stable EFI), `browse test:js` — a page whose <script>
...
K=os.path.dirname(os.path.abspath(__file__)); B=os.path.join(K,"build"); ESP=os.path.join(B,"esps4")
STABLE="/tmp/zeos-stable/BOOTZ.EFI"
QMP="/tmp/zeos-s4-qmp.sock"; SERSOCK="/tmp/zeos-s4-ser.sock"; SERLOG="/tmp/zeos-s4-serial.txt"
...
subprocess.run(["cp","/usr/share/OVMF/OVMF_VARS_4M.fd",os.path.join(B,"OVMF_VARSs4.fd")],check=True)
vault=os.path.join(B,"vaults4.img"); subprocess.run(["rm","-f",vault]); subprocess.run(["qemu-img","create","-f","raw",vault,"8M"],check=True)
for p in (QMP,SERSOCK,SERLOG):
    try: os.remove(p)
    except OSError: pass
qemu=subprocess.Popen(["qemu-system-x86_64",*accel_args(),"-machine","q35","-m","512M","-smp","4",
 "-drive","if=pflash,format=raw,readonly=on,file=/usr/share/OVMF/OVMF_CODE_4M.fd",
 "-drive",f"if=pflash,format=raw,file={B}/OVMF_VARSs4.fd","-drive",f"format=raw,file=fat:rw:{ESP}",
...
def shot(tag):
    o=f"/tmp/zeos-s4-{tag}.png"; qmp("screendump",filename=o,format="png")
...
time.sleep(1.5)
sw("\n");time.sleep(0.7)
sw("browse test:fetch\n");time.sleep(10.0)
shot("fetch")
print("=== SERIAL (browse test:fetch) ===")
print(ser().split("browse test:fetch")[-1][-700:])
