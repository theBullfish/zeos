...
K=os.path.dirname(os.path.abspath(__file__)); B=os.path.join(K,"build"); ESP=os.path.join(B,"espd3")
STABLE="/tmp/zeos-stable/BOOTZ.EFI"
QMP="/tmp/zeos-d3-qmp.sock"; SERSOCK="/tmp/zeos-d3-ser.sock"; SERLOG="/tmp/zeos-d3-serial.txt"
...
subprocess.run(["cp","/usr/share/OVMF/OVMF_VARS_4M.fd",os.path.join(B,"OVMF_VARSd3.fd")],check=True)
vault=os.path.join(B,"vaultd3.img"); subprocess.run(["rm","-f",vault]); subprocess.run(["qemu-img","create","-f","raw",vault,"8M"],check=True)
for p in (QMP,SERSOCK,SERLOG):
    try: os.remove(p)
    except OSError: pass
qemu=subprocess.Popen(["qemu-system-x86_64",*accel_args(),"-machine","q35","-m","512M",
 "-drive","if=pflash,format=raw,readonly=on,file=/usr/share/OVMF/OVMF_CODE_4M.fd",
 "-drive",f"if=pflash,format=raw,file={B}/OVMF_VARSd3.fd","-drive",f"format=raw,file=fat:rw:{ESP}",
...
def shot(tag):
    o=f"/tmp/zeos-d3-{tag}.png"; qmp("screendump",filename=o,format="png")
