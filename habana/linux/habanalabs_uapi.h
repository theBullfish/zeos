...
/*
 * Habana Labs UAPI — vendored from habanalabs-dkms 1.7.1-85
 * Source: /usr/src/habanalabs-dkms-1.7.1-85/include/uapi/misc/habanalabs.h on Temple
 *
 * NOTE: Goya is supported ONLY by the SDK 1.7.1-85 driver family. Upstream Linux
 * kernel UAPI (post-6.5) dropped Goya and drifted the hl_info_hw_ip_info struct
 * tail (added mme_enabled_mask, nic_ports_*, pdma_user_owned_ch_mask, etc.;
 * removed tpc_interrupt_id, rotator_enabled_mask, reserved_engines[8]). Building
 * habana_proof against the upstream header against an SDK driver returns garbage
 * in any post-card_name field. This SDK 1.7.1-85 header is the only correct one
 * for talking to /dev/hl* on any Goya host.
 *
 * SDK device_id semantics: the SDK driver returns hdev->pdev->device (raw PCI
 * dev ID) in hl_info_hw_ip_info.device_id (see goya_get_pci_id in
 * drivers/misc/habanalabs/goya/goya.c). Upstream UAPI returns a synthetic family
 * index (HL_DEVICE_GOYA=1, HL_DEVICE_GAUDI=3, ...). The two coincide for Goya
 * (both == 1) but DIVERGE for every other ASIC. The HL_DEVICE_* aliases below
 * are SDK-correct (PCI_IDS_* from drivers/misc/habanalabs/include/common/pci_ids.h)
 * so habana_proof's family switch works against the SDK driver. Cases for
 * GAUDI2B/2C/GAUDI3 are kept for source compat with upstream; the SDK driver
 * does not support those chips so those arms are dead on SDK 1.7.1-85.
 */
#define HL_DEVICE_GOYA          0x0001  /* PCI_IDS_GOYA */
#define HL_DEVICE_PLACEHOLDER   0x0002  /* upstream synthetic; unused by SDK */
#define HL_DEVICE_GAUDI         0x1000  /* PCI_IDS_GAUDI */
#define HL_DEVICE_GAUDI_SEC     0x1010  /* PCI_IDS_GAUDI_SEC */
#define HL_DEVICE_GAUDI2        0x1020  /* PCI_IDS_GAUDI2 */
#define HL_DEVICE_GAUDI2B       0x1030  /* PCI_IDS_GAUDI2_SEC (nearest SDK match) */
#define HL_DEVICE_GAUDI2C       0xFFFD  /* not in SDK 1.7.1-85 */
#define HL_DEVICE_GAUDI3        0xFFFC  /* not in SDK 1.7.1-85 */
 ...
 * @engine_id: engine id which caused the page fault.
 */
...
/* Maximum QP ID */
#define HL_NIC_MAX_CONN_ID	((1 << 11) - 1)
 ...
 * @swq_granularity: SWQ granularity [0 for 64B or 1 for 32B]
 ...
 * @coll_lag_idx: The index of the specific NIC within the LAG. Note that the
 *                advanced flag must be enabled in case it's being set.
 * @coll_last_in_lag: Is the specific NIC the last one within the collective LAG.
 ...
 * @sack_en: Enable Selective Acknowlegment (SACK)
 */
...
/**
 * enum hl_nic_cq_status - Status of the CQ, only HL_NIC_CQ_SUCCESS denotes a valid state.
 * @HL_NIC_CQ_SUCCESS: CQ has been and created and is operational
 * @HL_NIC_CQ_TIMEOUT: CQ polling has failed after the provided timeout and moved to invalid state
 * @HL_NIC_CQ_OVERFLOW: CQ is overflowed and moved to invalid state
 */
...
/**
 * struct hl_nic_cq_create_in - NIC ioctl opcode HL_NIC_OP_CQ_CREATE in param
...
/**
 * struct hl_nic_cq_create_out - NIC ioctl opcode HL_NIC_OP_CQ_CREATE out param
...
/**
 * struct hl_nic_cq_destroy_in - NIC ioctl opcode HL_NIC_OP_CQ_DESTROY in param
 * @handle: Handle of the CQ buffer
 */
...
/**
 * struct hl_nic_cq_update_consumed_cqes_in - NIC ioctl opcode HL_NIC_OP_CQ_UPDATE_CONSUMED_CQES in
 *                                            param
 * @handle: Handle of the CQ buffer
...
/**
 * struct hl_nic_cq_poll_wait_in - NIC ioctl in param for opcodes: HL_NIC_OP_CQ_WAIT,
 *                                                                 HL_NIC_OP_CQ_POLL
 * @handle: Handle of the CQ buffer
...
/**
 * struct hl_nic_cq_poll_wait_out - NIC ioctl out param for opcodes: HL_NIC_OP_CQ_WAIT,
 *                                                                   HL_NIC_OP_CQ_POLL
 ...
 * @HL_NIC_DB_FIFO_TYPE_COLL_OPS_SHORT: mode for short collective operations.
 * @HL_NIC_DB_FIFO_TYPE_COLL_OPS_LONG: mode for long collective operations.
 * @HL_NIC_DB_FIFO_TYPE_DWQ_LIN: mode for linear direct WQE submit.
 * @HL_NIC_DB_FIFO_TYPE_DWQ_MS: mode for multi-stride WQE submit.
 * @HL_NIC_DB_FIFO_TYPE_COLL_DIR_OPS_SHORT: mode for direct short collective
 *                                                             operations.
 * @HL_NIC_DB_FIFO_TYPE_COLL_DIR_OPS_LONG: mode for direct long collective
 *                                                             operations.
 */
...
/**
 * struct hl_nic_args - This structure is used as the arguments container for IOCTL HL_IOCTL_NIC
...
/**
 * struct hl_mme_shadow0_reg - one (offset, value) write to MME SHADOW_0
 * @offset:  CFG-region offset (must be in MME ARCH SHADOW_0 range [0xD0400, 0xD0800))
 * @value:   32-bit value to write
 */
struct hl_mme_shadow0_reg {
	__u32 offset;
	__u32 value;
};

#define HL_MME_SHADOW0_MAX_WRITES 128

/**
 * struct hl_mme_shadow0_args - IOCTL args for HL_IOCTL_PROGRAM_MME_SHADOW0
 * @writes_ptr: __u64 pointer to user array of struct hl_mme_shadow0_reg
 * @num_writes: number of entries (<= HL_MME_SHADOW0_MAX_WRITES)
 * @pad:        reserved, must be 0
 *
 * Kernel-side ioctl to write a small whitelisted set of MME tensor-descriptor
 * staging registers (SHADOW_0). MSG_LONG packets from user CBs cannot reach
 * SHADOW_0 (different AXI path); CP_DMA/MSG_PROT/WREG_BULK are blocked at
 * the CB parser. This ioctl provides a single sanctioned write path with
 * per-write range validation in [0xD0400, 0xD0800).
 */
struct hl_mme_shadow0_args {
	__u64 writes_ptr;
	__u32 num_writes;
	__u32 pad;
};
...
#define HL_IOCTL_PROGRAM_MME_SHADOW0	_IOWR('H', 0x08, struct hl_mme_shadow0_args)

/**
 * struct hl_mme_dbg_args - dump MME engine state via kernel-side MMIO read
 * @arch_regs:   __u32[16] readback of ARCH base regs starting at 0xD0000
 * @shadow_regs: __u32[16] readback of SHADOW_0 base regs starting at 0xD0400
 * @cmd_status: __u32 readback of MME_CMD register
 * @engine_status: __u32 readback of MME_ARCH_STATUS register
 */
struct hl_mme_dbg_args {
	__u32 arch_regs[16];
	__u32 shadow_regs[16];
	__u32 cmd_status;
	__u32 engine_status;
};

#define HL_IOCTL_MME_DBG	_IOR('H', 0x09, struct hl_mme_dbg_args)

/**
 * struct hl_mme_rreg_args - single-register read of an MME0 offset
 * @offset:   CFG-region offset in [0xD0000, 0xD0800)
 * @value:    output: 32-bit read value
 * @pad:      reserved, must be 0
 */
struct hl_mme_rreg_args {
	__u32 offset;
	__u32 value;
	__u32 pad;
};

#define HL_IOCTL_MME_RREG	_IOWR('H', 0x0a, struct hl_mme_rreg_args)

/**
 * struct hl_qm_rreg_args - timeout-protected register read for QM/MME diagnostics
 * @offset:      cfg-region offset, must be in [0xD0000, 0xD9000) (covers MME ARCH/SHADOW + QM region)
 * @timeout_us:  timeout in microseconds (0 -> use a 1000 us default)
 * @value:       output: 32-bit read value
 * @timed_out:   output: 1 if the read did not complete within timeout_us, else 0
 * @pad:         reserved, must be 0
 *
 * Designed for probing during stuck-engine windows without locking the
 * calling thread indefinitely.
 */
struct hl_qm_rreg_args {
	__u32 offset;
	__u32 timeout_us;
	__u32 value;
	__u32 timed_out;
...
#define HL_IOCTL_QM_RREG	_IOWR('H', 0x0d, struct hl_qm_rreg_args)

#define HL_COMMAND_START	0x01
#define HL_COMMAND_END		0x0e
