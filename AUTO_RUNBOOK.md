...
- last-two verified (most recent last): [K.3 (prod), L.3 (prod)]
- cycle log:
...
- L.4 DEFERRED (ghost-surface risk): wm_l4_selftest creates a surface + only ticks 4 after detach → leaves it mid-close; full teardown needs a wm per-frame call (wm.c:145-149 removes closing surface when spring settles), not just anim_tick. To do safely: tick to settle THEN call the wm update that removes it. Revisit.
...
- STATUS: RUNNING. selftest method proven for 16 items; Terminal real; G-series + K.3 + L.3 done. NEXT: L.4 (needs safe teardown — tick to settle THEN wm per-frame removal path wm.c:145-149, NOT anim_init), then click/multi-surface (C.12/13/14 need 2nd-surface setup via dock/double-click), M.7/M.8 (settings toggle), D.13 (dock reorder/poof drag), B.4 (frame_dt TSC compositor-embedded), A.4/A.6 (scheduler/persist measured). Keep watching for BUILD_MAP/ledger scope mismatches (found+fixed L.5).
