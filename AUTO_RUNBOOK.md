...
- last-two verified (most recent last): [G.4 (prod), L.5 (prod)]
- cycle log:
...
- STATUS: RUNNING. selftest method proven for 14 pure-logic/anim items; Terminal real; G-series done. NEXT (getting into harder remainder): L.3/L.4 (compositor anim-tick / surface open-close spring — may be selftest-able via anim state), K.3 (sigviz sine_pulse — if exposable), then click/multi-surface (C.12/13/14 need 2nd-surface setup), M.7/M.8 (settings toggle), D.13 (dock reorder/poof drag), B.4 (frame_dt TSC — compositor-embedded), A.4/A.6 (scheduler/persist — measured). Watch for more BUILD_MAP/ledger scope mismatches like L.5.
