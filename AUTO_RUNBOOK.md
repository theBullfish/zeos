...
- last-two verified (most recent last): [G.2 (prod), G.3 (prod)]
- cycle log:
...
- STATUS: RUNNING. selftest method proven for 12 pure-logic items; Terminal real. NEXT bucket-d via selftest: G.4 (dark/light theme tokens distinct+ordered, scheme round-trip — need theme accessors), B.4 (frame_dt TSC — compositor-embedded, harder). Then click/multi-surface (C.12/13/14 need 2nd-surface setup), M.7/M.8 (settings toggle), D.13 (dock reorder/poof via drag).
