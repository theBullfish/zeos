...
- last-two verified (most recent last): [P.3 (prod), P.2 (prod)]
- cycle log:
...
- STATUS: RUNNING. selftest method proven for 8 pure-logic items; Terminal real. NEXT bucket-d via selftest: B.4 (frame timing on TSC path), L.6 (scroll_phys momentum/rubber-band), G.1-G.4 (color tables/lerp/prompts), L.3/L.4/L.5 (anim-tick/surface-spring/menu — some observable). Interleave click/multi-surface (C.12/13/14).
