...
- last-two verified (most recent last): [G.3 (prod), G.4 (prod)]
- cycle log:
...
- STATUS: RUNNING. selftest method proven for 13 pure-logic items; Terminal real; G-series done. NEXT: B.4 (frame_dt TSC path — compositor-embedded, needs a different approach than selftest) OR pivot to remaining click/multi-surface (C.12 tabs, C.13 adjacency, C.14 parent/child — need 2nd-surface setup via dock/double-click), M.7/M.8 (settings toggle), D.13 (dock reorder/poof drag), A.4/A.6 (scheduler/persist — measured). Getting into the harder-setup remainder.
