...
- last-two verified (most recent last): [P.2 (prod), L.6 (prod)]
- cycle log:
...
- STATUS: RUNNING. selftest method proven for 9 pure-logic items; Terminal real. NEXT bucket-d via selftest: G.1-G.4 (persona color tables/dim/lerp/prompts, dark/light tokens), B.4 (frame_dt TSC path — harder, compositor-embedded). Then click/multi-surface (C.12/13/14 need 2nd-surface setup), M.7/M.8 (need settings toggle).
