...
- last-two verified (most recent last): [L.6 (prod), G.1 (prod)]
- cycle log:
...
- STATUS: RUNNING. selftest method proven for 10 pure-logic items; Terminal real. NEXT bucket-d via selftest: G.2 (persona_prompt distinct + cursor_select_colorway/cursor_get_accent switches — save/restore), G.3 (persona crossfade color_lerp mid strictly between src/dst — need public accessor), G.4 (dark/light theme tokens distinct+ordered), B.4 (frame_dt TSC — compositor-embedded, harder). Then click/multi-surface (C.12/13/14), M.7/M.8 (settings toggle).
